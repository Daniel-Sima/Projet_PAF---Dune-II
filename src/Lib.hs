...
prop_inv_Carte mapC = (M.foldr prop_inv_Terrain True mapC) && (M.foldrWithKey funct True mapC)
                        where 
                            funct k val acc = (M.foldrWithKey funct2 acc mapC)
...
get_val_Ressource :: Ressource -> Maybe Int 
